...
class EyeToHandCalibrator:
    ...
    def detect_charuco_pose(self, image: np.ndarray, camera_matrix: np.ndarray, dist_coeffs: np.ndarray) -> Optional[Tuple[np.ndarray, np.ndarray, np.ndarray, np.ndarray]]:
        ...
        # 4. ChArUco 코너가 충분히 검출된 경우 pose 추정
        if (charuco_corners is not None and charuco_ids is not None and 
            ...
            if objPoints is not None and imgPoints is not None and len(objPoints) >= 4:
                ...
                print(f"\rDEBUG: Not enough 2D-3D correspondences ({len(objPoints) if objPoints is not None else 0} < 4)", end="")
        ...
        try:
            ...
            if R_cam2base is not None:
                ...
                calibration_result = {
                    ...
                    "timestamp": datetime.now().isoformat(),
                    "description": "카메라 -> 로봇 베이스 변환 행렬 (Eye-to-Hand calibration, 미터 단위, 축보정 없음)",
    ...
    def _solve_eye_to_hand_calibration(self, 
        ...
        try:
            ...
            # 모든 포즈의 결과를 평균하여 최종 결과 계산
            R_cam2base_avg = np.mean(R_cam2base_list, axis=0)
            t_cam2base_avg = np.mean(t_cam2base_list, axis=0)
            
            # 회전 행렬을 직교 행렬로 정규화
            U, _, Vt = np.linalg.svd(R_cam2base_avg)
            R_cam2base_final = U @ Vt
            
            print(f"원본 t_cam2base: {t_cam2base_avg}")
            print(f"최종 t_cam2base: {t_cam2base_avg}")
            
            return R_cam2base_final, t_cam2base_avg  # 축보정 없이 원본 결과 반환
...
def main():
    ...
    calibrator = EyeToHandCalibrator(
        ...
        robot_baud=115200,
        save_dir="/home/ros/llm_robot/data/Calibration/Eye-to-Hand11",
        charuco_squares_x=7, 
        charuco_squares_y=5,   
        charuco_square_length=28.0, # mm (실제 측정값)
        charuco_marker_length=13.0, # mm (실제 측정값)
